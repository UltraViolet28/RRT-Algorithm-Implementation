...
def RRT_star(start, goal, obstacles):
    ...
    stride = 30 # value of jump towards random point
    proximity = 50 # Neighbourhood radius 
    ...
    for i in range(iteration):
        ...
        # Move Toward the random point
        m = (r_y - closest_node.y) / (r_x - closest_node.x)
        dx = 1/sqrt(1 + m**2)
        dy = m/sqrt(1 + m**2)
        new_x = closest_node.x + stride*dx
        new_y = closest_node.y + stride*dy
        ...
        # Cost of node
        # Get Close Proximity Nodes
        proximity_nodes = [closest_node]
        for node in nodes:
            if (dist(new_node,node)) < proximity:
                proximity_nodes.append(node)
        # print(len(proximity_nodes))
        
        # Choose Node with lowest cost to be the parent node
        parent_node = closest_node
        min_cost = parent_node.cost + dist(new_node, parent_node)
        for node in proximity_nodes:
            # if abs((node.y - new_node.y)*obstacle[0] - (node.x - new_node.x)*obstacle[1] + node.x*new_node.y - node.y*new_node.x) / dist(new_node, node) <= obstacle[2]:

            if node.cost + dist(new_node, node) < min_cost:
                parent_node = node
                min_cost = node.cost + dist(new_node, node)
        
        new_node.parentIs = parent_node
...
def main():
    ...
    pygame.display.set_caption("RRT Constant Step")
    running = True
    while running:
        ...
        pygame.draw.circle(screen, red, (start.x, start.y), 10)
