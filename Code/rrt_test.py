...
def RRT_star(start, goal, obstacles):
    ...
    proximity = 200 # Neighbourhood radius 
    bias = 100 # No Bias
...
# RRT_star(start, goal, K, delta_q, r):
#     tree = {start}
#     cost = {start: 0}
#     parent = {start: None}
...
#         q_near = nearest_node(q_rand, tree)
#         q_new = steer(q_near, q_rand, delta_q)
#         if obstacle_free(q_near, q_new, r):
#             neighbors = near_nodes(q_new, tree, r)
#             q_min = q_near
#             c_min = cost[q_near] + dist(q_near, q_new)
#             for q_near_n in neighbors:
#                 if obstacle_free(q_new, q_near_n, r):
#                     c_near_n = cost[q_near_n] + dist(q_near_n, q_new)
#                     if c_near_n < c_min:
#                         q_min = q_near_n
#                         c_min = c_near_n
#             tree.add(q_new)
...
#             cost[q_new] = c_min
#             for q_near_n in neighbors:
#                 if obstacle_free(q_near_n, q_new, r):
#                     c_near_n = cost[q_new] + dist(q_near_n, q_new)
#                     if c_near_n < cost[q_near_n]:
#                         parent[q_near_n] = q_new
#                         cost[q_near_n] = c_near_n
#             if goal_reached(q_new, goal):
#                 return path(tree, parent, q_new)
#     return None